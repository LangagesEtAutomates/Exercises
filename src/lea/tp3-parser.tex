...
\begin{exercice}[Langage de Dyck]
  ...
  On rappelle que le langage de Dyck sur les parenthèses est défini par la grammaire suivante. 
  $$\langle \{(,)\}, \{S\}, S, \{S \rightarrow S S \mid \varepsilon \mid ( S )\} \rangle.$$
  Les terminaux \verb|PAR_G| et \verb|PAR_D| sont déjà déclarés dans l'analyseur lexical
  pour reconnaître les parenthèses gauche et droite. 
  
  \begin{question}

  \item Modifiez la grammaire pour reconnaître le langage de Dyck.
  ...
  \item Modifiez les actions associées aux règles de la grammaire
    ...
    la chaîne. Par exemple, la chaîne \verb|(((()))(()))|
    devra retourner \verb|4|.

  \end{question}

  \emph{Remarque :} pour reconnaître le mot vide $\varepsilon$, il suffit de laisser le membre droit vide.
  Par exemple, la règle suivante reconnaît une chaîne vide ou un nombre. 
  \begin{lstlisting}
    expr ::= /* epsilon */            {: System.out.println("epsilon"); :}
    |        NUMBER:n                 {: System.out.println("nombre"); :}
    ;
  \end{lstlisting}
