...
\begin{exercice}[Mise en place]
  ...
  \begin{correction}
    \begin{lstlisting}[gobble=6]
      Buildfile: .../lea-tp2-lexer/build.xml
      clean:
      generate:
        [java] Reading "src/lea/Lexer.flex"                              // Ouverture du fichier
        [java] Constructing NFA : 43 states in NFA                       // Algo. de Thompson
        [java] Converting NFA to DFA :                                   // Déterminisation
        [java] ................                                          // (Progression)
        [java] 18 states before minimization, 13 states in minimized DFA // Minimisation
        [java] Writing code to "gen/lea/Lexer.java"                      // Transcription
      compile:
        [javac] Compiling 4 source files to .../lea-tp2-lexer/build
      BUILD SUCCESSFUL
      Total time: 1 second
    \end{lstlisting}
  \end{correction}
  ...
  \item Testez le programme. Vérifiez que tous les tests de \lstinline{Exo1Test} passent, et que les autres tests échouent. 
    \begin{lstlisting}[gobble=6]
      ant test
    \end{lstlisting}
  ...
  Si vous travaillez sous \emph{Eclipse}, vous pouvez lancer les tâches \texttt{ant}
  en utilisant les configurations préconfigurées. Allez dans ;
  \texttt{Run → Run Configurations...}, puis choisissez :
  \begin{itemize}
  \item \texttt{Launch Group → Compile-lea-tp2-lexer}
  \item \texttt{Launch Group → Test-lea-tp2-lexer}
  \item \texttt{Java Application → Run-lea-tp2-lexer}
  \end{itemize}
  Il est nécessaire de rafraîchir le projet (\texttt{F5} sur sa racine) après chaque compilation. 
\end{exercice}
...
\begin{exercice}[Reconnaissance de motifs]
  ...
  \item \texttt{Type} est l'un des types concrets qui implémentent l'interface \texttt{Token}.
    ...
    Cela permet d'utiliser le \emph{pattern matching} dans un bloc \lstinline|switch|,
    où le compilateur vérifie que chaque type de token est bien traité et transtype automatiquement
    le token dans une variable locale du bon type.
  ...
  \item les suites non vides de chiffres sont reconnues comme nombres
    et retournent un token \texttt{Token.Number} ;
  ...
  \end{question}
  \begin{correction}
    Le comportement est similaire au TP sur Gedit : si deux règles reconnaissent le même motif, la première règle a la priorité.
    Ici, il est important que les identifiants soient placés après les mots-clés pour que ``si'' (par exemple)
    soit reconnu comme un mot-clé, et pas comme un identifiant. 
  \end{correction}

  \begin{question}
  \item Observez la règle qui reconnaît les commentaires multi-lignes : \lstinline{"/*" ([^*] | "*"+ [^/])* "*"+ "/"}
    
    Pourquoi n'a-t-on pas simplement utilisé le motif \lstinline{"/*" . * "*/"}, plus lisible ?
    Donnez un exemple de texte qui serait reconnu par ce motif alors qu'il ne devrait pas l'être, 
    et un cas où ce motif échouerait alors qu'il devrait réussir.
  \end{question}
  \begin{correction}
    Le comportement du lexer est \emph{glouton} : il cherche toujours à reconnaître le plus long motif possible.
    Par exemple, ``\lstinline|/* c1 */ si /* c2 */|'' serait reconnu comme un seul commentaire avec cette règle.

    De plus, le point ne capture pas les passages à la ligne.
    Par exemple, ``\lstinline|/* l1 \\n l2 */|'' ne serait pas reconnu avec cette règle.
  \end{correction}
  ...
  \end{question}
  \begin{correction}
    Dans \texttt{lexer.flex} (le point médian peut être copié-collé depuis le sujet) :
    \begin{lstlisting}[gobble=6]
  ...
  \item Remplacez l'action de la règle de rattrapage par un appel à :\\
    \lstinline|error("caractère inattendu " + yytext())|.
  \end{question}
  \begin{correction}
    \begin{lstlisting}[gobble=6]
  ...
  \begin{question}
  \item Vérifiez que tous les tests des classes \texttt{Exo1Test} et \texttt{Exo2Test} sont désormais au vert. 
    Si certains tests de \texttt{Exo2Test} échouent encore, lisez attentivement les messages d'erreur
    et corrigez votre lexer.
  \end{question}
