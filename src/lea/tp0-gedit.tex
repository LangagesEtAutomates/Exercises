...
\hypersetup{
  pdftitle  = {Langages et Automates - Coloration syntaxique dans Gedit},
...
\begin{exercice}[Recherche par expressions rationnelles dans Gedit]
  ...
  \end{question}
  \begin{correction}
    \lstinline{\b} correspond à une chaîne vide à l'extrémité d'un mot. En l'ajoutant en début et en fin de l'expression rationnelle,
  ...
  \end{question}
  \begin{correction}
    Une réponse possible est \lstinline{//.*$}.
  \end{correction}
  ...
  \end{question}
  \begin{correction}
    Une réponse possible est \lstinline{\b([1-9][0-9]*|0)\b}.
  \end{correction}
...
\begin{exercice}[Coloration syntaxique]
  ...
  \end{question}
  \begin{correction}
    Les mot-clés comme ``si'', ``alors'', etc sont à la fois reconnus par ``identifier'' et par ``keyword''.
    Il faut placer ``keyword'' en premier car les règles de priorité impliquent que la première qui soit retenue dans ce cas.
  ...
  \begin{correction}
    \begin{lstlisting}[gobble=6]
      <context id="keyword" style-ref="def:keyword">
        <keyword>algorithme</keyword>
        <keyword>début</keyword>
        <keyword>si</keyword>
        <keyword>alors</keyword>
  ...
  \end{question}
  \begin{correction}
    Ne pas oublier de rajouter le contexte à ``\texttt{algo}''
    \begin{lstlisting}[gobble=6]
  ...
  \begin{correction}
    Ne pas oublier de rajouter le contexte à ``\texttt{algo}''
    \begin{lstlisting}[gobble=6]
      <context id="char" style-ref="def:character">
        <match extended="true">
	  \'([^\'\\]|[\\].)\'
        </match>
      </context>
