...
syntaxiques pour Java, conceptuellement proche de Bison ou Yacc développés pour C/C++.
...
\begin{exercice}[Mise en place]
  ...
  Un exemple de programme utilisant CUP est fourni dans le dépôt.
  ...
  \item Clonez le dépôt :
    \begin{lstlisting}[gobble=6, language=bash]
      git clone https://github.com/LangagesEtAutomates/lea-tp3-parser
      cd lea-tp3-parser
    \end{lstlisting}
  ...
  \item Générez le lexer JFlex, l'analyseur CUP et compilez le projet :
    \begin{lstlisting}[gobble=6, language=bash]
      ant compile
    \end{lstlisting}
    Vérifiez que CUP a bien généré des fichiers
    \lstinline{build/lea/Parser.java} et 
    \lstinline{build/lea/Terminal.java}.

  \item Lancez les tests :
    \begin{lstlisting}[gobble=6, language=bash]
      ant test
    \end{lstlisting}
    Les tests de \texttt{LexerTest} doivent passer, et les autres peuvent échouer.
  ...
  \item Lancez le programme :
    ...
    Il s'agit d'une calculatrice en notation polonaise inversée, c'est-à-dire où les opérateurs sont utilisés en mode postfixe.
    Testez-le en saisissant quelques expressions dans le terminal : 
    \begin{lstlisting}[gobble=6]
      42            // Sortie attendue : 42
      1 2 +         // Sortie attendue : 3   (1+2)
      2 3 * 4 5 * + // Sortie attendue : 26  (2*3 + 4*5)
    \end{lstlisting}
  \end{question}
  ...
  retourne un tel objet, contenant le type du token (par exemple
  \lstinline|Terminal.NUMBER| ou \lstinline|Terminal.PLUS|) et, éventuellement,
  une valeur associée (par exemple un entier). Le parseur CUP
  ...
  \begin{question}
  \item L'analyseur lexical est déjà donné dans le fichier \lstinline{src/lea/Lexer.flex}.
  ...
  \begin{center}
    \begin{tikzpicture}[x=50mm, y=6mm]
      ...
      \path[-latex] (main.west |- parse.north)   edge node[above]{\texttt{parseInt()}}           (parse.north east) {};
      ...
      \path[-latex] (yylex2.south east)          edge node[above]{\texttt{Symbol(NUMBER, 2)}} (parse.west |- yylex2.south) {};
      \path[-latex] (parse.west |- yylex3.north) edge node[above]{\texttt{yylex()}}           (yylex3.north east) {};
      \path[-latex] (yylex3.south east)          edge node[above]{\texttt{Symbol(PLUS)}}      (parse.west |- yylex3.south) {};
  ...
  \item \lstinline|terminal PLUS, MINUS;| déclare les symboles terminaux
    ...
    CUP génère automatiquement un fichier \lstinline|build/lea/Terminal.java|
  ...
  \item
    \lstinline|non terminal Integer expr;| déclare le non-terminal
  ...
  $S \rightarrow N \mid S S +$, où $N$ représente un entier :
  \begin{lstlisting}
  ...
  \item Testez le programme avec les expressions suivantes,
    et dessinez les arbres de dérivation des résultats :
    \begin{lstlisting}
      1 2 4 + +  // Sortie attendue : 7
      10 2 + 3 * // Sortie attendue : 36
    \end{lstlisting}
  \end{question}
  \begin{correction}
  \vspace{-3mm}
  \begin{center}
    \begin{tikzpicture}[tree, x=10mm, y=7mm]
      \tree{\texttt{expr}}{
        \tree{\texttt{expr}}{\tree{$1$}{}}
        \tree{\texttt{expr}}{
          \tree{$2$}{}
          \tree{$4$}{}
          \tree{$+$}{}
...
\begin{exercice}[Langage de Dyck]

  On rappelle que le langage de Dyck sur les crochets est défini par la grammaire suivante. 
  $$\langle \{[,]\}, \{S\}, S, \{S \rightarrow \varepsilon \mid S [ S ] \} \rangle.$$
  Les terminaux \verb|CRO_G| et \verb|CRO_D| sont déjà déclarés dans l'analyseur lexical
  pour reconnaître les crochets gauche et droit. 
  
  \begin{question}
  \item Modifiez la grammaire pour que le non-terminal \texttt{dyck} reconnaisse le langage de Dyck.
    
  \item Modifiez les actions associées aux règles de la grammaire
    pour calculer le nombre maximal de crochets imbriqués dans
    la chaîne. Par exemple, l'analyse de la chaîne \verb|[[[[]]][[]]]|
    devra retourner \verb|4|.
  \end{question}

  \emph{Remarque :} pour reconnaître le mot vide $\varepsilon$, il suffit de laisser le membre droit vide.
  Par exemple, la règle \texttt{dyck} du code de départ reconnaît le langage neutre. 
  \begin{lstlisting}
    dyck ::= /* epsilon */ ;
  \end{lstlisting}
...
\begin{correction}
  \begin{lstlisting}
    dyck ::= /* epsilon */               {: RESULT = 0; :}
           | dyck:d1 CRO_G dyck:d2 CRO_D {: RESULT = Math.max(d1, d2 + 1); :}
    ;
  \end{lstlisting}
\end{correction}
...
\begin{exercice}[Grammaire de la calculatrice]
  ...
  On cherche à implémenter les opérateurs suivants (du moins prioritaire au plus prioritaire) :
  ...
  il devra produire $1$ si les deux opérandes sont égaux, et $0$ sinon.

  \begin{question}

  \item Modifiez votre calculatrice pour prendre en charge
...
\begin{correction}
  \begin{lstlisting}
    /* Terminaux ajoutés pour les booléens */
    terminal PLUS, MOINS, MULTIPLIE, PAR_G, PAR_D, EGAL, OU, ET, NON;
    terminal Integer ENTIER;
     
    /* Non-terminaux : tous transportent un entier */
    non terminal Integer input, expr, disjonction, conjonction, negation;
    non terminal Integer comparaison, somme, produit, facteur, terme;
    ...
    expr ::=
        expr:e OU disjonction:d            {: RESULT = (e != 0 || d != 0) ? 1 : 0; :}
      | disjonction:d                      {: RESULT = d; :}
    ;
     
    // ET
    disjonction ::=
        disjonction:d ET conjonction:c     {: RESULT = (d != 0 && c != 0) ? 1 : 0; :}
      | conjonction:c                      {: RESULT = c; :}
    ;
     
    // NON
    conjonction ::=
        NON negation:n                     {: RESULT = (n==0) ? 1 : 0; :}
      | negation:n                         {: RESULT = n; :}
    ;
     
    // EGALITÉ (=) : Plus prioritaire que la logique, moins que l'arithmétique
    negation ::=
        comparaison:c1 EGAL comparaison:c2 {: RESULT = c1==c2 ? 1 : 0; :}
      | comparaison:c                      {: RESULT = c; :}
    ;
     
    /* --- Opérations Arithmétiques --- */
     
    // SOMME (+, -) : Associatifs à gauche
    comparaison ::=
        comparaison:c PLUS somme:s         {: RESULT = c + s; :}
      | comparaison:c MOINS somme:s        {: RESULT = c - s; :}
      | somme:s                            {: RESULT = s; :}
    ;
     
    // PRODUIT (*)
    somme ::=
        somme:s MULTIPLIE produit:p        {: RESULT = s * p; :}
      | produit:p                          {: RESULT = p; :}
    ;
     
    // MOINS UNAIRE : Plus prioritaire que *, moins que les parenthèses
    produit ::=
        MOINS produit:p                    {: RESULT = -p; :}
      | facteur:f                          {: RESULT = f; :}
    ;
     
    // TERME DE BASE (Parenthèses, Nombres)
    facteur ::=
